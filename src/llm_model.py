...
def get_answer(
    ...
    system_prompt = (
        ...
        "on what you already know about similar projects "
        "or libraries, even when "
        "it looks relevant. If the sources do not "
        "contain the answer, say so "
        "plainly instead of guessing. Be concise: answer "
        "the question that was "
        "asked, in a few sentences, without adding setup"
        "steps or examples that "
        "are not in the sources.")
    message = [{"role": "system", "content": system_prompt}]
    message.append({"role": "user", "content": context})
    text = cast(str, llm._tokenizer.apply_chat_template(
        message,
